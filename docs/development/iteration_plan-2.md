...
- フィーチャバッファ: Phase 7（統合テスト）は優先度を下げ、必要に応じて次イテレーションに延期可能
- スケジュールバッファ: 22h（Phase 1-6 で不測の事態に対応）
...
**受け入れ基準**:
- [ ] Cake Coverage タスクが正常に動作する
- [ ] coverage.cobertura.xml が生成される
- [ ] 現在のカバレッジ率が表示される
...
**受け入れ基準**:
- [ ] UnvalidatedOrder が実装されている
- [ ] 生データ（string, decimal）を受け取れる
- [ ] テストが実装されている
...
**受け入れ基準**:
- [ ] validateOrder 関数が実装されている
- [ ] すべてのバリデーションルールが適用される
- [ ] エラーメッセージが適切
- [ ] テストが実装されている（成功ケース + エラーケース）
...
**受け入れ基準**:
- [ ] priceOrder 関数が実装されている
- [ ] 価格計算が正しい
- [ ] GetProductPrice スタブが実装されている
- [ ] テストが実装されている
...
**受け入れ基準**:
- [ ] acknowledgeOrder 関数が実装されている
- [ ] 適切なイベントが生成される
- [ ] SendOrderAcknowledgment スタブが実装されている
- [ ] テストが実装されている
...
gantt
    ...
    section デイリー活動
    デイリースタンドアップ         :daily, 2025-02-03, 10d

    section レビュー
    イテレーションレビュー         :review, 2025-02-16, 2h
...
| **11** | 2/16 (日) | レビュー (1h), レトロ (0.75h) | レビュー (1h), レトロ (0.75h) | 3.5h |

**合計**: 75.5h（計画 78h に対し 97%）
...
| リスク | 影響度 | 確率 | 軽減策 |
|--------|--------|------|--------|
| Phase 5-6 の見積もり過小 | 中 | 中 | バッファ 22h を活用、Phase 7 の一部を延期 |
...
- [ ] プロパティベーステストが実装されている（該当する場合）
- [ ] テストカバレッジが 80% 以上（**イテレーション 2 新規追加**）

✅ **ドキュメント**:
- [ ] 型定義にドキュメントコメントがある
- [ ] 関数にドキュメントコメントがある
- [ ] README や設計ドキュメントが更新されている

✅ **CI/CD**:
- [ ] GitHub Actions でビルドが成功する
- [ ] GitHub Actions でテストが成功する
- [ ] カバレッジレポートが生成される（**イテレーション 2 新規追加**）

✅ **Story 1.1 特有の基準**:
- [ ] PlaceOrder ワークフローが動作する
- [ ] 正常系の統合テストが成功する
- [ ] ドメインモデル設計ドキュメントが最新化されている
...
- **チーム理想時間**: 100h
- **バッファ**: 22h（22%）
- **目標完了率**: 100%（Phase 1-6）、80%（Phase 7 はオプション）
...
**機能完成度**:
- [ ] Story 1.1 の受け入れ基準をすべて満たす
- [ ] PlaceOrder ワークフローが E2E で動作する
- [ ] 制約付き型 13 種類が実装されている
- [ ] 複合値オブジェクト 5 種類が実装されている
- [ ] エンティティ 3 種類が実装されている

**品質指標**:
- [ ] テストカバレッジ 80% 以上
- [ ] すべてのテストが成功（単体 + プロパティ + 統合）
- [ ] Fantomas フォーマットチェック合格
- [ ] FSharpLint 警告ゼロ
