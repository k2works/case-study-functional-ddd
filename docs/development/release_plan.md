...
| イテレーション | ストーリー | SP | 累計 SP | 残 SP |
|---------------|----------|-----|---------|-------|
| **Iter 0** | 環境構築 (E-1 ~ E-8) | - | 0 | 87 |
| **Iter 1** | 1.1 (一部) | 8 | 8 | 79 |
| **Iter 2** | 1.1 (完了), 1.2 (一部) | 13 | 21 | 66 |
| **Iter 3** | 1.2 (完了), 1.3 | 10 | 31 | 56 |
| **Iter 4** | 1.4, 3.1 | 11 | 42 | 45 |
| **Iter 5** | 4.1, 5.1 (一部) | 8 | 50 | 37 |
| **Iter 6** | 5.1 (完了) | 2 | 52 | 35 |
| **Iter 7** | 5.2, 5.3 | 8 | 60 | 27 |
| **Iter 8** | 3.3 | 8 | 68 | 19 |
| **Iter 9** | 2.1, 2.2 | 8 | 76 | 11 |
| **Iter 10** | 2.3 | 8 | 84 | 3 |
| **Iter 11** | 3.2 (一部) | 3 | 87 | 0 |
| **Iter 12** | 4.2, バッファ | 3 | 87 | 0 |

**注**: イテレーション 0 は環境構築専用で、ストーリーポイントは含まれません。詳細は [イテレーション 0 計画](iteration_plan-0.md) を参照してください。
...
| リスク | 影響度 | 確率 | 軽減策 | 対応者 |
|--------|--------|------|--------|--------|
| F# の学習曲線 | 高 | 高 | スパイクイテレーション、ペアプログラミング | 開発チーム |
| Entity Framework InMemory の制約 | 中 | 中 | PostgreSQL 移行オプションを設計 | アーキテクト |
| Heroku の制約 | 中 | 低 | スケーリング戦略を事前検証 | インフラ担当 |
| 外部サービス連携 | 中 | 中 | モックサービスでの開発、リトライロジック | 開発チーム |

### スケジュールリスク

| リスク | 影響度 | 確率 | 軽減策 |
|--------|--------|------|--------|
| ベロシティの過大見積もり | 高 | 中 | 保守的見積もり (80%)、バッファ確保 |
| 要件変更 | 中 | 高 | イテレーション単位での調整、バックログ管理 |
| チームメンバー不在 | 中 | 低 | ペアプログラミング、ドキュメント整備 |
...
- [イテレーション 0 計画 (環境構築)](iteration_plan-0.md)
